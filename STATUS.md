...
**Corpus extended to 54 days — committed, deployed, and now plumbing-verified.** Ran `corpus:generate -- --count 40` (`scholar` capped early at 54, the new bottleneck tier — `magister`/`etymologus` reached 99/88, contrary to the expectation that the hardest tiers would run dry first), then `corpus:compile -- --start 2026-08-18 --include-unreviewed`. Result: **2026-08-18 → 2026-10-10 (54 days)**, up from the previous 19. **This is a meaningfully bigger review-debt jump than the earlier "49/95 unreviewed" note** — the compile output flagged 390 unverified candidates in the eligible pool; only a small fraction of the full 270 shipped words (54 days × 5 tiers) were ever hand-reviewed, so treat this 54-day corpus as almost entirely unverified, not just "half reviewed" like before. Still fine for continued dogfooding with the small trusted group per the staging-flag precedent in `bulk-generator-brief.md`/`STATUS.md`, but the gap to a real launch-ready corpus (`LAUNCH.md`'s hard gate: re-compile *without* `--include-unreviewed` from a fully reviewed set) just got substantially wider. **`LAUNCH.md` plumbing gate confirmed Aug 26** via the browser Network tab (not the function logs — see correction below): live response for `2026-08-27` returned `"source": "corpus"` and `"servedFrom": "corpus"`, real words at every tier.

**Correction to `LAUNCH.md`: "check the function logs for `servedFrom: corpus`" was never actually possible.** The corpus-serving success path in `get-daily-puzzle.js` had no `console.log` call at all — `servedFrom` only ever appeared in the HTTP response body, never in Netlify's function logs. (The `generated`/on-demand path *does* log, which is why the instinct to look at logs wasn't crazy — it's just that the log only fires on the fallback/failure path, not the success path you're actually trying to confirm.) Fixed: added one `console.log` line so the success path logs too, matching the other branches. Not yet deployed — small, low-risk, no rush.
...
- ~~Corpus extension — commit, push, deploy~~ — **done, Aug 26.** Committed and deployed. **Plumbing gate confirmed** via Network tab (`servedFrom: "corpus"`, real words for 2026-08-27). Small follow-up, not urgent: a one-line `console.log` fix to `get-daily-puzzle.js` so this check can actually be done via function logs in the future, as `LAUNCH.md` describes — written, not yet deployed.
...
public/corpus.json                  compiled puzzles the game serves (live: 54 days through 2026-10-10 as of Aug 26, ~390 unreviewed in the pool — plumbing gate confirmed via Network tab)
